...
export function generateStaticParams() {
  return getAllPosts("ja").map((p) => ({ slug: p.slug }));
}
...
export default async function BlogPostPage({
  ...
  const related = getRelatedPosts(post.slug, "ja", 4);
    ...
    <PageShell lang="ja">
      ...
      <article className="bg-white px-5 py-14">
        <div className="mx-auto max-w-2xl">
          <Link href="/ja/blog" className="text-sm font-bold text-brand hover:underline">
            ← ブログ
          </Link>
          ...
          <div className="mt-4 flex items-center gap-2 text-xs font-bold text-brand">
            ...
            <span className="text-navy/40">
              <time dateTime={postDateISO(post)}>{formatPostDate(post, "ja")}</time>{isPostUpdated(post) && <span className="ml-1">(更新)</span>} · {post.readMinutes}分で読めます
            </span>
          </div>
          ...
          {post.faq && post.faq.length > 0 && (
            <div className="mt-12">
              <h2 className="text-xl font-black text-navy">よくある質問</h2>
          ...
          <div className="mt-12 rounded-2xl bg-gradient-to-br from-[#1a2332] to-[#105d9e] p-8 text-center text-white">
            <p className="text-lg font-bold text-gold">ビジョンを植えれば、必ず実がなる 🌱</p>
            <a
          ...
          {related.length > 0 && (
            <section className="mt-14">
              <h2 className="text-lg font-extrabold text-navy">関連記事</h2>
